...
function Auth({ mode, onToggle, onDone }: any) {
  ...
  const go = () => {
    ...
    onDone({ email:f.email, firstName:mode==="signup"?f.firstName:f.email.split("@")[0], age:mode==="signup"?parseInt(f.age)||25:25 });
  };
  return (
      ...
      <Glass style={{padding:20,marginBottom:14,background:user.isPremium?"rgba(245,158,11,0.05)":T.bgCard}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div><p style={{fontSize:15,fontWeight:700,color:T.tx}}>MystiPremium</p><p style={{fontSize:12,color:T.txM}}>4,99€ / mois</p></div>
          </div>
...
export default function App() {
  ...
  const like = (prof: any) => {
    if(Math.random()>0.3){ const m={id:`m-${Date.now()}`,prof,mc:0,msgs:[]}; setMatches(p=>[m,...p]); setPopup(prof); }
  };
  ...
  const sendMsg = (mid: string, text: string) => {
    ...
    setMatches(p=>p.map(up)); setActive((p: any)=>p&&p.id===mid?{...p,msgs:[...p.msgs,msg],mc:p.mc+1}:p);
    setTimeout(()=>{
      ...
      setMatches(p=>p.map(up2)); setActive((p: any)=>p&&p.id===mid?{...p,msgs:[...p.msgs,r],mc:p.mc+1}:p);
  ...
  return (
    <div style={{fontFamily:"'Satoshi',system-ui,sans-serif",background:T.bg,color:T.tx,minHeight:"100vh",maxWidth:480,margin:"0 auto",position:"relative",overflow:"hidden"}}>
      ...
      <AnimatePresence mode="wait">
        ...
        {scr==="auth"&&<Auth key="a" mode={authMode} onToggle={()=>setAM((m: string)=>m==="login"?"signup":"login")} onDone={(d: any)=>{setUser(d);setScr("onboarding");}}/>}
        {scr==="onboarding"&&<Onboard key="o" userData={user} onDone={(u: any)=>{setUser({...u,isPremium:false});setScr("discovery");}}/>}
        ...
        {scr==="profile"&&user&&<ProfileView key="p" user={user} onPrem={()=>setUser((p: any)=>({...p,isPremium:!p.isPremium}))} onLogout={()=>{setUser(null);setMatches([]);setActive(null);setScr("splash");}}/>}
      </AnimatePresence>
      {isApp&&scr!=="chat"&&(
